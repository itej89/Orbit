...
\begin{tikzpicture}[
  ...
  rtr/.style={box, fill=orange!18, minimum width=2.4cm, minimum height=1.1cm},
  ctl/.style={box, fill=yellow!25, minimum width=2.0cm},
  cli/.style={box, fill=green!12},
  arr/.style={-{Stealth[length=5pt]}, thick},
  darr/.style={-{Stealth[length=5pt]}, thick, dashed, gray}
]
% Clients
\node[cli] (cli) {Clients\\[-2pt]\scriptsize Poisson arrivals};

% Router
\node[rtr, right=0.9cm of cli] (rtr) {
  \textbf{Orbit Router}\\[-2pt]
  \scriptsize FastAPI proxy\\[-2pt]
  \scriptsize RR \,|\, LOR \,|\, PO2\\[-2pt]
  \scriptsize MPC-PO2 \,|\, Kalman-PO2
};

% Servers
\node[srv, right=0.9cm of rtr, yshift=0.45cm]  (s0) {Server 0 (fast)\\[-2pt]\scriptsize GPU 0, seqs=64};
\node[srv, right=0.9cm of rtr, yshift=-0.45cm] (s1) {Server 1 (slow)\\[-2pt]\scriptsize GPU 1, seqs$\leq$64};

% Controller
\node[ctl, below=0.45cm of rtr] (ctl) {\scriptsize MPC: QP/100\,ms\\[-2pt]\scriptsize Kalman filter};

% Arrows
\draw[arr] (cli) -- (rtr);
\draw[arr] (rtr.east) |- (s0.west);
\draw[arr] (rtr.east) |- (s1.west);
\draw[darr] (s0.south) -- ++(0,-0.25) -| (ctl.east);
\draw[darr] (s1.north) -- ++(0, 0.15) -| (ctl.east);
\draw[darr] (ctl.west) -- (rtr.south);
\end{tikzpicture}
\caption{Orbit architecture. The FastAPI router applies a configurable
  policy; MPC-PO2 additionally runs an OSQP quadratic program every
  100\,ms using EMA service-rate estimates; Kalman-PO2 maintains a
  per-server Kalman filter on observed request latency (dashed = feedback).}
\label{fig:arch}
\end{figure}
