...
are consistently 15--20\% worse than PO2 and LOR at steady state, and this
gap does not close even after 10,000 requests ($\approx$42 minutes of traffic).
Kalman-PO2 shows a transient advantage in short runs (327\,ms vs.\ 386\,ms
mean, $10\times$ lower P99) by eliminating PO2's burst variance, but this
benefit does not persist.
We identify the root cause --- reactive PO2 already exploits queue-depth
signals optimally at moderate load, and both MPC and Kalman measure
\emph{experienced} latency rather than \emph{intrinsic} capacity --- and
derive concrete redesign directions: throughput-based rate estimation,
TTFT-driven feedback, and hybrid reactive-predictive control. We present
this as a cautionary case study in applying control-theoretic methods to
LLM serving.
\end{abstract}
...
Orbit is a FastAPI service between clients and a pool of vLLM servers.
In \emph{single-pool} mode, each request goes to one server performing
both prefill and decode. In \emph{PD-disaggregated} mode, the router
selects a prefill and a decode server per request, injecting peer addresses
into the request ID consumed by vLLM's KV-transfer connector.
Four routing policies share the same infrastructure: \textbf{RR}
(round-robin), \textbf{LOR} (min inflight count), \textbf{PO2} (sample
two, pick lower weighted-inflight score), and \textbf{MPC-PO2} (PO2
with MPC-computed per-server weights).
...
\text{s.t.} \quad
  & q_{k+1} = q_k + \Delta t \cdot
    \tfrac{\lambda_a p_i - \hat{\mu}_i}{\bar{q}},\quad
    q_{k+1} \geq 0,\quad
...
\texttt{gpu-memory-utilization=0.15}). Heterogeneity is simulated via two
vLLM servers on separate GPUs:
\textbf{Server 0 (fast):} \texttt{max-num-seqs=64} (fixed);
\textbf{Server 1 (slow):} \texttt{max-num-seqs} $\in \{64,32,16,8\}$,
...
(1) At moderate load, PO2 is a robust baseline that is difficult to
beat; spend complexity on signal selection, not controller design.
(2) Latency-based routing is more discriminative than queue-depth in
short-run scenarios but requires uncertainty gating to remain stable.
(3) A feedback signal that encodes intrinsic capacity (e.g.\ time-to-first-token,
or throughput-normalized completion rate) is needed for sustained gains.

\textbf{Open directions:} uncertainty-gated fallback to PO2 when
latency estimates overlap; TTFT-specific routing for prefill-decode
disaggregated topologies; and evaluation under overload where PO2's
implicit balancing itself breaks down.
